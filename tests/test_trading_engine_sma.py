...
class TestTradingEngineSMA:
    """Test cases for the TradingEngine with SimpleMovingAverageStrategy."""
    ...
    async def setup(self):
        ...
        self.strategy_service = Mock(spec=StrategyService)
        
        # Set up async methods as AsyncMock
        self.data_service.get_candles = AsyncMock()
        self.data_service.close_all = AsyncMock()
        self.execution_service.place_order = AsyncMock()
        self.execution_service.get_account_info = AsyncMock()
        self.execution_service.get_positions = AsyncMock()
        self.execution_service.get_orders = AsyncMock()
        self.execution_service.close_all = AsyncMock()
        ...
        self.strategy_service.get_all_strategies.return_value = {
            "SimpleMovingAverageStrategy": self.strategy
        }
        ...
        self.strategy_config = {
            ...
            "long_period": 10,
            "position_size": 0.1,
            "signal_callback": self.trading_engine._process_signal
        }
        self.strategy.initialize(self.strategy_config)
        
        # Mock the _execute_signal method to avoid actual order execution
        self.trading_engine._execute_signal = AsyncMock()
        ...
        self.execution_service.get_account_info.return_value = account
        ...
        self.execution_service.get_positions.return_value = positions
        
        # Set up mock orders data
        self.execution_service.get_orders.return_value = []
    ...
    @pytest.mark.asyncio
    async def test_sma_buy_signal_generation(self, setup):
        ...
        for candle in candles:
            self.strategy.on_candle(candle, "btcusd", "1h", "gemini")
        
        # Reset the mock to track new calls
        self.trading_engine._execute_signal.reset_mock()
        ...
        assert self.trading_engine._execute_signal.called, "No signal was executed"
        
        # Get the signal that was passed to execute_signal
        signal = self.trading_engine._execute_signal.call_args[0][0]
    ...
    @pytest.mark.asyncio
    async def test_sma_sell_signal_generation(self, setup):
        """Test that the SMA strategy generates a sell signal when short MA crosses below long MA."""
        # First, set up a position
        position = Position(
            symbol="btcusd",
            quantity=0.5,  # We have a position to sell
            entry_price=10000.0,
            mark_price=10000.0,
            unrealized_pnl=0.0
        )
        self.strategy.current_position = 0.5  # Set position in strategy
    ...
    @pytest.mark.asyncio
    async def test_trading_engine_processes_signal(self, setup):
        ...
        # Process the signal
        self.trading_engine._process_signal(signal)
        
        # Verify the signal was passed to execute_signal
        self.trading_engine._execute_signal.assert_called_once()
        executed_signal = self.trading_engine._execute_signal.call_args[0][0]
    ...
    @pytest.mark.asyncio
    async def test_position_update_notification(self, setup):
        """Test that position updates are correctly processed by the strategy."""
        # Mock the strategy's on_position_update method
        original_on_position_update = self.strategy.on_position_update
        self.strategy.on_position_update = Mock()
        ...
        await self.trading_engine._notify_position_update(position, "gemini")
        
        # Verify the strategy received the position update
        self.strategy.on_position_update.assert_called_once_with(position, "gemini")
        
        # Restore original method
        self.strategy.on_position_update = original_on_position_update
    
    @pytest.mark.asyncio
    async def test_account_update_notification(self, setup):
        """Test that account updates are correctly processed by the strategy."""
        # Mock the strategy's on_account_update method
        original_on_account_update = self.strategy.on_account_update
        self.strategy.on_account_update = Mock()
        ...
        await self.trading_engine._notify_account_update(account, "gemini")
        
        # Verify the strategy received the account update
        self.strategy.on_account_update.assert_called_once_with(account, "gemini")
        
        # Restore original method
        self.strategy.on_account_update = original_on_account_update
    
    @pytest.mark.asyncio
    async def test_order_update_notification(self, setup):
        """Test that order updates are correctly processed by the strategy."""
        # Mock the strategy's on_order_update method
        original_on_order_update = self.strategy.on_order_update
        self.strategy.on_order_update = Mock()
        ...
        await self.trading_engine._notify_order_update(order, "gemini")
        
        # Verify the strategy received the order update
        self.strategy.on_order_update.assert_called_once_with(order, "gemini")
        
        # Restore original method
        self.strategy.on_order_update = original_on_order_update
